...
class Blender(core.View):
  ...
  def render_single_rgba(self,
    ...
    if target_dir is not None:
      rgba_save_dir = kb.as_path(target_dir)
    else:
      rgba_save_dir = self.scratch_dir / "images"
    with RedirectStream(stream=sys.stdout, disabled=self.verbose):
      ...
      if image_name is None:
        bpy.context.scene.render.filepath = str(
            rgba_save_dir / f"{prefix}_{frame-frame_start:04d}.png")
      else:
        bpy.context.scene.render.filepath = str(
            rgba_save_dir / f"{image_name}.png")
    ...
    if not return_segmentation:
      ...
      from_dir = kb.as_path(self.scratch_dir)
      exr_frames = sorted((from_dir / "exr").glob("*.exr"))
      # print('blender.py exr_frames:', exr_frames)
      assert len(exr_frames) == 1
      source_layers = blender_utils.get_render_layers_from_exr(exr_frames[0])
      post_processor = self.post_processors['segmentation']
      segmentation = post_processor(source_layers, self.scene) # nd.array
      # print('blender.py segmentation:', segmentation.shape)
      return bpy.context.scene.render.filepath, np.expand_dims(segmentation, 0)

  # anwen hu 2022/8/19
  def render_rgbas(self,
             frames: Optional[Sequence[int]] = None,
             ignore_missing_textures: bool = False,
             target_dir: str=None,
             prefix:str='frame',
    ...
    """Renders all frames (or a subset) of the animation and save dir.
    ...
    with RedirectStream(stream=sys.stdout, disabled=self.verbose):
      for frame_nr in frames:
        ...
        # (but for exr it does, so we only adjust the png path)
        bpy.context.scene.render.filepath = str(
              rgba_save_dir / f"{prefix}_{frame_nr-frame_start:04d}.png")
        bpy.ops.render.render(animation=False, write_still=True)
        logger.info("Rendered frame '%s'", bpy.context.scene.render.filepath)
    return str(rgba_save_dir)
  

  def render_rgbas_detail(self,
             frames: Optional[Sequence[int]] = None,
             ignore_missing_textures: bool = False,
             target_dir: str=None,
             prefix:str='frame',
  ...
  def postprocess(
    ...
    for exr_filename, png_filename in zip(exr_frames, png_frames):
      ...
      # print('blender.py ', source_layers.keys())
