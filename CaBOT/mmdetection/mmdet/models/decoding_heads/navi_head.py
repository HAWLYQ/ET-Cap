...
class SingleNavigatorHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        super(SingleNavigatorHead, self).__init__(init_cfg)
        ...
        self.move_cls_out_channels = move_classes
    ...
    def _init_layers(self):
        ...
        self.move_fc_cls = Linear(self.embed_dims, self.move_cls_out_channels)
        ...
        self.fc_reg = Linear(self.embed_dims, 3) # predict move step, yaw and pitch angles
    ...
    def forward_single(self, x, seq_masks):
        ...
        all_move_cls_scores = self.move_fc_cls(outs_dec) # [nb_dec, bs, seq, 7]
        all_yaw_cls_scores = self.yaw_fc_cls(outs_dec) # [nb_dec, bs, seq, 2]
        all_pitch_cls_scores = self.pitch_fc_cls(outs_dec) # [nb_dec, bs, seq, 2]
        
        all_step_angles_preds = self.fc_reg(self.activate(self.reg_ffn(outs_dec))).sigmoid() # [nb_dec, bs, seq, 3]
        all_move_steps_preds = all_step_angles_preds[:,:,:,0]
        all_yaw_angles_preds = all_step_angles_preds[:,:,:,1]
        all_pitch_angles_preds = all_step_angles_preds[:,:,:,2]
        
        return all_move_cls_scores, all_yaw_cls_scores, all_pitch_cls_scores, all_move_steps_preds, all_yaw_angles_preds, all_pitch_angles_preds

    
    @force_fp32(apply_to=('all_move_cls_scores_list', 'all_yaw_cls_scores_list','all_pitch_cls_scores_list'))
    def loss(self,
             all_move_cls_scores_list,
             all_yaw_cls_scores_list,
             all_pitch_cls_scores_list,
             all_move_step_preds_list,
        ...
        all_move_cls_scores = all_move_cls_scores_list[-1]
        all_yaw_cls_scores = all_yaw_cls_scores_list[-1]
        all_pitch_cls_scores = all_pitch_cls_scores_list[-1]
        all_move_step_preds = all_move_step_preds_list[-1]
        ...
        num_dec_layers = len(all_move_cls_scores)

        all_move_gt_labels_list = [gt_move_labels_list for _ in range(num_dec_layers)]
        ...
        all_gt_move_steps_list = [gt_move_steps_list for _ in range(num_dec_layers)]
        ...
        # multiple-layer loss
        losses_move_cls, losses_yaw_cls, losses_pitch_cls, losses_move_step, losses_yaw_angle, losses_pitch_angle = multi_apply(self.loss_single,
                                                                                                                 all_move_cls_scores, all_move_gt_labels_list,
                                                                                                                 all_yaw_cls_scores, all_yaw_gt_labels_list,
                                                                                                                 all_pitch_cls_scores, all_pitch_gt_labels_list,
                                                                                                                 all_move_step_preds, all_gt_move_steps_list,
        ...
        loss_dict['loss_move_cls'] = losses_move_cls[-1]
        loss_dict['loss_yaw_cls'] = losses_yaw_cls[-1]
        loss_dict['loss_pitch_cls'] = losses_pitch_cls[-1]
        loss_dict['loss_move_step'] = losses_move_step[-1]
        loss_dict['loss_yaw_angle'] = losses_yaw_angle[-1]
        loss_dict['loss_pitch_angle'] = losses_pitch_angle[-1]
        # loss from other decoder layers
        """num_dec_layer = 0
        for loss_move_cls_i, loss_yaw_cls_i, loss_pitch_cls_i, loss_move_step_i, loss_yaw_angle_i, loss_pitch_angle_i  in zip(losses_move_cls[:-1],
                                                                                                                                losses_yaw_cls[:-1],
                                                                                                                                losses_pitch_cls[:-1],
                                                                                                                                losses_move_step[:-1],
                                                                                                                                losses_yaw_angle[:-1],
                                                                                                                                losses_pitch_angle[:-1]
                                                                                                                                ):
            loss_dict[f'd{num_dec_layer}.move_loss_cls'] = loss_move_cls_i
            loss_dict[f'd{num_dec_layer}.yaw_loss_cls'] = loss_yaw_cls_i
            loss_dict[f'd{num_dec_layer}.loss_pitch_cls'] = loss_pitch_cls_i
            loss_dict[f'd{num_dec_layer}.loss_move_step'] = loss_move_step_i
            loss_dict[f'd{num_dec_layer}.loss_yaw_angle'] = loss_yaw_angle_i
            loss_dict[f'd{num_dec_layer}.loss_pitch_angle'] = loss_pitch_angle_i
            num_dec_layer += 1"""
        return loss_dict

    def loss_single(self,
                    move_cls_scores, move_gt_labels,
                    yaw_cls_scores, yaw_gt_labels,
                    pitch_cls_scores, pitch_gt_labels,
                    move_step_preds, gt_move_steps,
        ...
        losses_move_cls = self.loss_cls(move_cls_scores, move_gt_labels, seq_mask)
        ...
        losses_move_step = self.loss_reg(move_step_preds, gt_move_steps, seq_mask)
        ...
        return losses_move_cls, losses_yaw_cls, losses_pitch_cls, losses_move_step, losses_yaw_angle, losses_pitch_angle

    # over-write because img_metas are needed as inputs for bbox_head.
    def forward_train(self,x, x_mask, gt_move_labels,gt_move_steps, gt_yaw_labels, gt_yaw_angles, gt_pitch_labels, gt_pitch_angles):
        ...
        loss_inputs = outs + (gt_move_labels, gt_yaw_labels, gt_pitch_labels, gt_move_steps, gt_yaw_angles, gt_pitch_angles)
