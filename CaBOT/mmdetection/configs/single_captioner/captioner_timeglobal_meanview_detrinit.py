...
model = dict(
    ...
    cap_head=dict(
        type='SingleCaptionerRegionTimeHead',
        encoder_width=256,
        min_dec_len=5,
        max_dec_len=max_cap_len,
        dec_beam_size=3,
        repetition_penalty=1.0,
        med_config='/root/code/mmdetection/configs/single_captioner/2croatt_config.json',
        ),
    cap_input_feat_type='TimeGlobal_MeanViewLocal',
    backbone_init=dict(type='DETR', checkpoint='/root/code/ET-Cap/CaBOT/mmdetection/tools/work_dirs/embodied_detr/latest.pth'),
    frozen_backbone=frozen_backbone,
    neck_init=None,
    frozen_neck=frozen_neck,
    # test_cfg=dict(eval_mode='test'),
    )
...
data = dict(
    samples_per_gpu=32, # 8 x 4 GPU(11G); 16 x 2 GPU (19G)
    workers_per_gpu=4,
    train=dict(
        type=dataset_type,
