...
scene = kb.Scene(resolution=(256, 256), frame_start=1, frame_end=2)
...
for frame in range(scene.frame_start, scene.frame_end + 1):
  ...
  scene.camera.position = (7, 7, 7)
  scene.camera.look_at((1, 1, 1))
...
tmp_save_path = renderer.render_single_rgba(frame=scene.frame_start, target_dir="output/",image_name='test')
"""tmp_save_path = renderer.render_rgbas_detail(frames=range(scene.frame_start, scene.frame_start+3),
                                      target_dir="output/", prefix='rgba')"""
end_time = time.time()
